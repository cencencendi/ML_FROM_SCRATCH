...
def _log_odds(proba: float):
    """
    Convert a probability to log odds using the logit function.

    Parameters
    ----------
    proba : float
        The probability value, which must be between 0 and 1 (exclusive).

    Returns
    -------
    float
        The log odds calculated from the probability.
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        for estimator_idx in range(self.n_estimators):
            if estimator_idx == 0:
                ...
                # For classification problem, replace leaf value with log(odds) of the probability
                # where probability defined as mean(y) -> already calculated when growing tree.
                if self.update_terminal_regions:
                    probability = self.estimators[estimator_idx].tree_.value
                    self.estimators[estimator_idx].tree_.value = _log_odds(proba=probability)
                continue
            ...
            # Optionally update terminal regions for specialized loss functions --> for classification problem
            if self.update_terminal_regions:
